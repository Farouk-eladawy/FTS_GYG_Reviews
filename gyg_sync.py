...
def customer_review_text_only(text):
    if not text:
        return ""
    return str(text).split(AI_REPLY_MARKER, 1)[0].strip()

def normalize_review_text(text):
    return re.sub(r"\s+", " ", customer_review_text_only(text)).strip().lower()

def reviews_are_same(existing, incoming):
    existing_norm = normalize_review_text(existing)
    incoming_norm = normalize_review_text(incoming)
    if not existing_norm or not incoming_norm:
        return False
    if existing_norm == incoming_norm:
        return True
    if min(len(existing_norm), len(incoming_norm)) < 40:
        return False
    return existing_norm in incoming_norm or incoming_norm in existing_norm

def booking_already_synced(cache, booking_ref):
    if not cache or not booking_ref:
        return False
    prefix = f"{booking_ref}_"
    return any(key == booking_ref or str(key).startswith(prefix) for key in cache)
...
def sync_to_airtable(review_data):
    ...
    try:
        with _AIRTABLE_WRITE_LOCK:
            ...
            if data.get("records"):
                record_id = data["records"][0]["id"]
                existing_fields = data["records"][0].get("fields") or {}
                existing_review = str(existing_fields.get("Customer Review") or "").strip()
                existing_rating = existing_fields.get("GYG Rating")
                incoming_review = str(review_data.get("review_comment") or "").strip()
                incoming_rating = review_data.get("general_rating")

                if existing_review and not reviews_are_same(existing_review, incoming_review):
                    log(
                        f"Skipping overwrite for {booking_ref}: Airtable already has a different Customer Review (first write kept).",
                        "WARNING",
                    )
                    return "skipped_existing"

                fields = {}
                if not existing_review and incoming_review:
                    fields["Customer Review"] = incoming_review[:100000]
                elif existing_review:
                    log(f"Customer Review already set for {booking_ref}. Keeping first write.", "INFO")

                if existing_rating in (None, ""):
                    if incoming_rating not in (None, ""):
                        fields["GYG Rating"] = int(incoming_rating)
                elif incoming_rating not in (None, "") and int(existing_rating) != int(incoming_rating):
                    log(
                        f"Skipping rating overwrite for {booking_ref}: Airtable has {existing_rating}, incoming is {incoming_rating}.",
                        "WARNING",
                    )

                if not existing_fields.get("Review Date") and review_data.get("review_date"):
                    fields["Review Date"] = review_data["review_date"]

                if not fields:
                    log(f"No Airtable fields to update for {booking_ref}.", "INFO")
                    return "already_set"
        ...
        for page_num in range(MAX_PAGES):
            ...
            for index, card in enumerate(review_cards):
                try:
                    ...
                    review_hash = f"{booking_ref}_{rating}"
                    if booking_already_synced(cache, booking_ref):
                        log(f"Skipping Airtable sync for {booking_ref}: first write already cached.", "INFO")
                    else:
                        log(f"Syncing review for {booking_ref}...")
                        sync_status = sync_to_airtable(review_data)
                        if sync_status in ("updated", "already_set"):
                            cache[review_hash] = 1
                            save_cache(cache)
                            processed_records += 1
                        elif sync_status == "skipped_existing":
                            log(f"Did not cache {booking_ref} as synced because incoming review did not match the first write.", "WARNING")
                        elif sync_status == "not_found":
                            log(f"Airtable record missing for {booking_ref}. Will retry on a later cycle.", "WARNING")
                        else:
...
if __name__ == "__main__":
    ...
    if ACCOUNT_COUNT > 1:
        ...
        run_account_worker(1, once=once)
>>>>>>> 0e583e8b (Update from local environment)
